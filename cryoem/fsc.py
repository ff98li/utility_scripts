"""
    Modified from https://github.com/zhonge/cryodrgn/blob/master/analysis_scripts/fsc.py
    A Python script for calculating the Fourier Shell Correlation
    between two cryo-EM density maps
"""
...
def main(args):
    ...
    if args.plot:
        ...
        plt.axvline(x = 1 / (2 * args.Apix),
                    color = 'b', ls = ':',
                    label = 'physical Nyquist limit')
        plt.axhline(y = 0.5, color = 'aqua', ls = ':')
        plt.axhline(y = 0.143, color = 'purple', ls = ':')
        #plt.show()
        plt.legend(loc = 0)
        fig.savefig(args.o + '.png', dpi = fig.dpi)
